// Painel injetado dentro da própria página do x.com (dentro de uma Shadow
// DOM, para não vazar nem sofrer interferência do CSS do site).
(function () {
  const HOST_ID = "x-unfollow-checker-host";
  ...
  shadow.innerHTML = `
    ...
    <div class="panel" id="panel" hidden>
      ...
      <div class="warn">
        ...
        fila nem loop automático. A extensão impõe uma pausa mínima entre
        sincronizações e entre unfollows, e trava sozinha por um tempo ao
        detectar qualquer sinal de limite vindo do X.
      </div>
  ...
  const ERROR_MESSAGES = {
    NOT_LOGGED_IN: "Você precisa estar logado no x.com.",
    RATE_LIMITED: "O X limitou as requisições temporariamente. A extensão vai ficar pausada por um tempo antes de tentar de novo.",
    ACCOUNT_LIMITED: "O X sinalizou uma limitação de ação nesta conta. Pare de usar a extensão até ela ser removida.",
  };
  ...
  function render() {
    ...
    for (const u of filtered) {
      ...
      uname.href = `https://x.com/${encodeURIComponent(u.username)}`;
      uname.target = "_blank";
  ...
  async function loadFromStorage() {
    ...
    if (xUnfollowCheckerResult) {
      currentResult = xUnfollowCheckerResult;
      render();
    }
  }
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    ...
    if (msg?.type === "SYNC_PROGRESS") {
      ...
      if (p.stage === "me") statusLine.textContent = "Conectado à sua conta. Carregando listas...";
